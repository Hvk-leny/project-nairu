...
# ==============================================================================
    ...
    # 🟢 CORRECTION INITIALISATION : Empêche le crash AttributeError
            ...
            if st.checkbox("Voir la gestion des comptes"):
                ...
                for nom, infos in list(data_totale["comptes"].items()):
                    ...
                    if nom not in ["admin_nairu_leny", "admin_nairu_eliott", "leny", "eliott", "exemple"]:
                        ...
                        # 🔥 AFFICHAGE DES MDPs : Ajout du mot de passe en clair dans les infos admin
                        st.caption(f"IP : {infos.get('ip', '0.0.0.0')} | Email : {infos.get('email', 'N/A')} | MDP : `{infos.get('password', 'N/A')}`")
